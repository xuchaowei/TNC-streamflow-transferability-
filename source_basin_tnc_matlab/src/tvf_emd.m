...
function imf = tvf_emd(x,THRESH_BWR,BSP_ORDER)
%input
% x: input signal x(t)
%output
% imf: resulting intrinsic mode functions
 ...
 THRESH_BWR=THRESH_BWR; % inst bandwidth  threshold (stopping criterion)
 MAX_IMF=10; % max output imfs
 BSP_ORDER=BSP_ORDER; % b-spline order
 end_flag=0;
